...
- 假设让**右指针**往**左**走，可能找到更大的面积
  ...
  - 若 $y_{1} < x_{0}$，则 $min(x_{0}, y{1}) = y_{1} < min(x_{0}, y_{0})$   

    若 $y_{1} > x_{0}$，则 $min(x_{0}, y{1}) = x_{0} = min(x_{0}, y_{0})$   

    所以， <center> $ min(x_{0}, y_{1}) \leq min(x_{0}, y_{0}) $</center>
  - 显然， <center> $ y_{1} - x_{0} < y_{0} - x_{0} $</center>
  - 所以， <center> $ S_{1} = min(x_{0}, y_{1}) * (y_{1} - x_{0}) \leq S_{0} $</center>
